const AggregatePeerTable=(()=>{
    ...
    function table(){return `<table class="wide-report-table"><colgroup>${[11,20,13,6,6,6,6,6,6,6,14].map(n=>`<col style="width:${n}%">`).join('')}</colgroup><thead><tr>${['ISIN','Fondo','Categoría','TER %','Media TER %','Δ TER pp','Score','Media score','Δ Score','Peers','Coste y calidad relativa'].map(label=>`<th>${label}</th>`).join('')}</tr></thead><tbody>${rows().map(r=>`<tr><td>${escapeHtml(r.record.isin||r.original)}${r.record.isin!==r.original?`<br><small>Origen: ${escapeHtml(r.original)}</small>`:''}</td><td>${escapeHtml(r.record.name||'-')}${r.manual?'<br><small>TER manual</small>':''}</td><td>${escapeHtml(r.record.category||'-')}</td><td>${number(r.record.ter)}</td><td>${number(r.meanTer)}</td><td>${delta(r.deltaTer)}</td><td>${number(r.record.score)}</td><td>${number(r.meanScore)}</td><td>${delta(r.deltaScore)}</td><td>${r.count}/${r.total}</td><td>${r.assessment}</td></tr>`).join('')||'<tr><td colspan="11">Carga y analiza las posiciones agregadas.</td></tr>'}</tbody></table>`;}
