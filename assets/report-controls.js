const ReportControls=(()=>{
    ...
    function init(){
        ...
        aggregatePanel.innerHTML=`<h3>Contenido del informe agregado</h3><div class="workspace-report-options">${Object.entries(managerBlocks).map(([key,label])=>`<label><input type="checkbox" data-manager-block="${key}" checked>${label}</label>`).join('')}</div>`;
    ...
    async function chart(rows){
        ...
        try{await FinancialVisuals.newPlot(div,traces,{xaxis:{title:metrics[o.x],automargin:true},yaxis:{title:metrics[o.y],automargin:true},margin:{t:80,l:100,r:340,b:100},legend:{orientation:'v',x:1.05,y:1,font:{size:10}}},{staticPlot:true});return await Plotly.toImage(div,{format:'png',width:1100,height,scale:1.5});}finally{Plotly.purge(div);div.remove();}
    }
