const ReportControls=(()=>{
    ...
    const blocks={positions:'Tabla de posiciones',changes:'Tabla de cambios',classes:'Clases mas baratas',details:'Detalle de metricas',savings:'Grafica de ahorro',overview:'Comparacion global',pairs:'Graficas por sustitucion'};
    const initial={blocks:Object.fromEntries(Object.keys(blocks).map(k=>[k,true])),x:'risk5',y:'ret5',size:'ter'};
    const manager={range:'YTD',start:'',end:''};
    ...
    function init(){
        ...
        const period=document.createElement('div');period.className='workspace-settings';period.innerHTML='<label>Periodo del informe<select id="managerReportRange"><option value="YTD">YTD</option><option value="1Y">1 ano</option><option value="3Y">3 anos</option><option value="5Y">5 anos</option><option value="ALL">Todo el historico</option><option value="CUSTOM">Fechas personalizadas</option></select></label><label id="managerStartLabel" hidden>Desde<input id="managerReportStart" type="date"></label><label id="managerEndLabel" hidden>Hasta<input id="managerReportEnd" type="date"></label>';
    ...
    function state(){return JSON.parse(JSON.stringify({initial:options(),manager}));}
    ...
    function classTable(rows){return `<table><thead><tr><th>Origen</th><th>Comprobacion de clases</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${escapeHtml(r.current?.name||r.isin||'-')}<br>${escapeHtml(r.isin||r.current?.isin||'')}</td><td>${shareClassSummary(r.current)}</td></tr>`).join('')}</tbody></table>`;}
    ...
    async function chart(rows){
        ...
        for(const [side,label] of [['current','Origen'],['proposed','Propuesta']]){
            const records=rows.map(r=>r[side]).filter(r=>r&&Number.isFinite(r[o.x])&&Number.isFinite(r[o.y]));
            if(records.length)traces.push({type:'scatter',mode:'markers',name:label,x:records.map(r=>r[o.x]),y:records.map(r=>r[o.y]),text:records.map(r=>`${r.name} · ${r.isin}`),marker:{symbol:side==='current'?'circle':'diamond',size:records.map(r=>o.size==='fixed'||!Number.isFinite(r[o.size])?14:Math.max(10,Math.min(32,10+Math.sqrt(Math.abs(r[o.size]))*4)))},hovertemplate:'%{text}<br>X %{x:.2f}<br>Y %{y:.2f}<extra>%{fullData.name}</extra>'});
        }
        if(!traces.length)return '';
        const div=document.createElement('div');div.style.cssText='position:fixed;left:-12000px;width:980px;height:520px';document.body.append(div);
        try{await FinancialVisuals.newPlot(div,traces,{xaxis:{title:metrics[o.x]},yaxis:{title:metrics[o.y]},margin:{t:30,l:80,r:30,b:90},legend:{orientation:'h',y:-.2}},{staticPlot:true});return await Plotly.toImage(div,{format:'png',width:980,height:520,scale:1.5});}finally{Plotly.purge(div);div.remove();}
    }
    ...
    async function generateInitial(){
        ...
        const section=(title,html)=>`<section class="block"><h2>${escapeHtml(title)}</h2>${html}</section>`;
        ...
        if(b.overview)parts.push(section('Mapa global de la propuesta',figure(await chart(rows),caption())));
        if(b.details||b.pairs)for(const row of rows){await waitForUiFrame();parts.push(section(`${row.isin} - Analisis de sustitucion`,(b.details?fundProposalMetricReportTable(row):'')+(b.pairs?figure(await chart([row]),caption()):'')));}
        ...
        if(manual.length)parts.push(section('Ajustes manuales',manual.map(text=>`<p>${escapeHtml(text)}</p>`).join('')));
    ...
    return {init,state,options,manager,periodRows,validatePeriod,aggregateComparison,aggregateClasses,generateInitial};
})();
ReportControls.init();
