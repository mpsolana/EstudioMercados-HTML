const FundSelector = (() => {
    ...
    function rows(state) {
        ...
        const content = visible.map(rec => `<button type="button" class="fund-option" data-option-search="${esc(rec.isin)}" onclick="${esc(state.selectCall(rec.isin))}; closeCompactFundComboboxes();"><strong>${esc(rec.name || rec.isin)}</strong><small>${esc(rec.isin)} · ${esc(rec.category || 'Sin categoria')} · Score ${Number.isFinite(rec.score) ? rec.score.toFixed(2) : '-'} · TER ${Number.isFinite(rec.ter) ? rec.ter.toFixed(2) + '%' : '-'}</small></button>`).join('');
