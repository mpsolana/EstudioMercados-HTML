...
(async()=>{
    ...
    try{
        ...
        for(const key of ['savings','overview','pairs','details','changes','classes','flows','waterfall','comparison','concentration'])await page.locator(`[data-report-block="${key}"]`).uncheck();
        ...
        const traces=await page.evaluate(async()=>{const original=FinancialVisuals.newPlot,seen=[];FinancialVisuals.newPlot=(el,data,...args)=>{seen.push(...data.map(t=>t.name));return original(el,data,...args);};try{await generateFundProposalReport();return seen;}finally{FinancialVisuals.newPlot=original;}});
        assert.ok(traces.some(s=>s.startsWith('Peer group')));assert.ok(traces.some(s=>s.startsWith('Media categoría')));assert.ok(traces.some(s=>s.startsWith('Origen')));assert.ok(traces.some(s=>s.startsWith('Propuesta')));
        assert.ok(await page.evaluate(()=>fundProposalState.reportHtml.includes('TER (%) / Score')));
        ...
        for(const key of ['overview','manual','methodology','sources'])await page.locator(`[data-report-block="${key}"]`).uncheck();
        await page.locator('[data-report-block="classes"]').check();
        await page.evaluate(()=>generateFundProposalReport());
        const minimal=await page.evaluate(()=>fundProposalState.reportHtml);
        const opportunityRows=await page.evaluate(()=>{const doc=new DOMParser().parseFromString(fundProposalState.reportHtml,'text/html');return [...doc.querySelectorAll('h2')].find(h=>h.textContent==='Clases mas baratas').parentElement.querySelectorAll('tbody tr').length;});assert.equal(opportunityRows,1);
        for(const heading of ['Ajustes manuales','Metodologia y cobertura','Origen de los datos: proxies','Expediente '])assert.equal(minimal.includes(heading),false);
        await page.evaluate(()=>{
        ...
        assert.equal(await page.locator('#managerReportRange option[value="1Y"]').textContent(),'1 año');
        ...
        for(const checkbox of await page.locator('[data-manager-block]').all())await checkbox.uncheck();
        await page.locator('[data-manager-block="impact"]').check();
        const managerReport=await page.evaluate(async()=>{const before=buildManagerExposureSection;buildManagerExposureSection=()=>{throw new Error('Excluded exposure generated');};try{await generateManagerReport();return managerReportHtml;}finally{buildManagerExposureSection=before;}});
        assert.ok(managerReport.includes('Impacto de los cambios'));assert.equal(managerReport.includes('Clases equivalentes y costes'),false);assert.equal(managerReport.includes('Expediente '),false);assert.equal(managerReport.includes('El agregado comercial'),false);
        await page.screenshot({path:path.join(out,'aggregate-controls.png')});
        await page.locator('[data-manager-block="positions"]').check();await page.locator('[data-manager-block="santa"]').check();
        const wideReport=await page.evaluate(async()=>{fundUniverseState.records[0].name='Santalucía Renta Fija Internacional Clase A EUR';aggregateScoringResults[0].position.name=fundUniverseState.records[0].name;await generateManagerReport();const doc=new DOMParser().parseFromString(managerReportHtml,'text/html');return {parts:managerReportHtml.includes('Parte 1 de'),columns:[...doc.querySelectorAll('table.wide-report-table')].map(t=>t.rows[0].cells.length)};});
        assert.equal(wideReport.parts,false);assert.ok(wideReport.columns.includes(12));assert.ok(wideReport.columns.includes(11));
        const wideDownload=page.waitForEvent('download');
        const pdfTables=await page.evaluate(async()=>{const create=pdfMake.createPdf;let definition;pdfMake.createPdf=function(d,...args){definition=d;return create.call(this,d,...args);};try{await ReportDesign.downloadPdf(managerReportHtml,'Agregado-horizontal.pdf');const widths=[];const visit=n=>{if(!n||typeof n!=='object')return;if(n.table)widths.push(n.table.widths);Object.values(n).forEach(visit);};visit(definition.content);return {orientation:definition.pageOrientation,widths};}finally{pdfMake.createPdf=create;}});
        await(await wideDownload).saveAs(path.join(out,'agregado-horizontal.pdf'));
        const widths=pdfTables.widths.find(w=>w.length===12),actualWidth=w=>typeof w==='number'?w:w._calcWidth??w.width;
        assert.equal(pdfTables.orientation,'landscape');assert.ok(actualWidth(widths[2])>actualWidth(widths[6]));
        const approval=await page.evaluate(()=>{const previousUniverse=fundUniverseState,previousApproved=approvedFundsState;try{const a={isin:'ES0000000001',name:'Aurum Global Bond Fund A EUR Acc',manager:'Aurum',category:'Bonos',aum:1000,ter:1},b={...a,isin:'ES0000000002',name:'Aurum Global Bond Fund I EUR Acc',ter:.3};fundUniverseState={records:[a,b],isinIndex:new Map([[a.isin,a],[b.isin,b]])};approvedFundsState={records:[a],isinIndex:new Map([[a.isin,a]])};const off=isFundApprovedRecord(b);const checkbox=document.getElementById('extendApprovedClasses');checkbox.checked=true;checkbox.dispatchEvent(new Event('change',{bubbles:true}));const on=isFundApprovedRecord(b),badge=approvedFundBadge(b);checkbox.checked=false;checkbox.dispatchEvent(new Event('change',{bubbles:true}));return {off,on,badge,none:shareClassSummary(b)};}finally{fundUniverseState=previousUniverse;approvedFundsState=previousApproved;}});
        assert.equal(approval.off,false);assert.equal(approval.on,true);assert.match(approval.badge,/por familia/);assert.match(approval.none,/No se han identificado/);assert.doesNotMatch(approval.none,/insuficientes/);
